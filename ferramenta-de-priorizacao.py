# app.py (versão com correção final para a formatação da private_key)
...
# --- Configuração da Conexão com o Google Sheets (COM A CORREÇÃO) ---

@st.cache_resource(ttl=600)
def connect_gsheets():
    """Conecta-se ao Google Sheets, corrigindo a formatação da private_key."""
    try:
        # Pega no dicionário de credenciais dos segredos do Streamlit
        creds_dict = st.secrets["gcp_service_account"].to_dict()
        
        # AQUI ESTÁ A CORREÇÃO MÁGICA:
        # Recriamos a chave privada, substituindo os caracteres de escape
        # pelas quebras de linha reais que a biblioteca de criptografia espera.
        creds_dict['private_key'] = creds_dict['private_key'].replace('\\n', '\n')
        
        # Passamos o dicionário de credenciais já corrigido para o gspread
        ...
        st.error(f"Erro ao conectar ao Google Sheets. Detalhes: {e}")
...
def ler_projetos(worksheet):
    try:
        ...
        st.error("A aba 'Sheet1' não foi encontrada na sua folha de cálculo. Verifique o nome.")
        return pd.DataFrame()
    except Exception:
        return pd.DataFrame()
...
def calcular_notas(df):
    ...
    colunas_db = {'alinhamento': MAPA_ALINHAMENTO, 'ebitda': MAPA_EBITDA, 'complexidade': MAPA_COMPLEXIDADE, 'custo': MAPA_CUSTO, 'dependencia': MAPA_DEPENDENCIA}
    for col, mapa in colunas_db.items():
        df[f'score_{col}'] = df[col].map(mapa)
...
def classificar_projetos(df):
    ...
    return df, ponto_corte, ponto_crote
...
if submitted:
    if worksheet:
        ...
        st.sidebar.error("Falha na conexão com o Google Sheets. O projeto não foi salvo.")
...
if worksheet:
    ...
    if not df_projetos.empty:
        ...
        fig.add_annotation(x=esf_corte/2, y=imp_crote/2, text="Projetos Rápidos", showarrow=False, font=dict(color="gray", size=10))
